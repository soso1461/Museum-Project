    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            if re.search(r"\b(exit|quit)\b", transcript, re.I):
                ...
                print("파일 입력을 종료합니다.")
                stable()
                playSound("CommandFinish")
                time.sleep(5)
                break
            elif transcript.strip() == "clear":
                playSound("CommandReset")
                print("모든 리스트를 초기화합니다.")
                is_start = False
                word_list = []
            else:
                if is_start == False : # is_start 가 False라면 add 라고 추가해줘야함
                    if transcript.strip() == "add":
                        is_start = True
                        playSound("CommandInputStart")
                        print("add 모드로 시작합니다.")
                        word_list.append("add")
                else:                   # start mode 이므로 단어 삽입
                    if transcript.strip() == "delete" :
                        word_list.pop()
                        playSound("CommandCancel")
                        print("delete 실행 !!")
                    else :
                        playSound("CommandInput")
